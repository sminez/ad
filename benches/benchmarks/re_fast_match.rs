...
fn inputs(n: usize) -> (GapBuffer, Regex) {
    ...
    let r = Regex::compile("fo*bar").unwrap();
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    let (mut gb, mut r) = inputs(100);
    group.bench_function("leading 100 normal", |b| {
        b.iter(|| assert!(r.match_gb(black_box(&mut gb)).is_some()))
    });
    group.bench_function("leading 100 fast", |b| {
        b.iter(|| assert!(r.match_gb_fast(black_box(&mut gb)).is_some()))
    });

    let (mut gb, mut r) = inputs(1000);
